...
Settings loadSettings(Settings previousSettings, char* filePath) {
	...
	xmlNode* gameChild = rootElement->chilren;
	while (gameChild) {
		if (gameChild->type == XML_ELEMENT_NODE) {
			if (!strcmp(gameChild->name, "next_turn"))
				settings.playingColor = parseColor(gameChild->content);
			else if (!strcmp(gameChild->name, "game_mode"))
				settings.gameMode = (*gameChild->content - '0'; //should be '1' or '2'
			else if (!strcmp(gameChild->name, "difficulty"))
				settings.minimaxDepth = parseDifficulty(gameChild->content);
			else if (!strcmp(gameChild->name, "user_color"))
				settings.userColor = parseColor(gameChild->content);
			else if (!strcmp(gameChild->name, "board"))
